...
Regex regex = new Regex(@"capacity (-?\d+), durability (-?\d+), flavor (-?\d+), texture (-?\d+), calories (-?\d+)");
...
foreach (string line in lines) {
    ...
    foreach (Match match in matches) {
        ...
        int cal = int.Parse(match.Groups[5].Value);
        Ingredient i = new Ingredient(cap, dur, fla, tex, cal);
        ingr.Add(i);
    }
}
...
for (int a = 0; a < 101; a++) {
    for (int b = 0; b < 101 - a; b++) {
        for (int c = 0; c < 101 - a - b; c++) {
            int d = 100 - a - b - c;
            int cal = a * ingr[0].cal + b * ingr[1].cal + c * ingr[2].cal + d * ingr[3].cal;
            if (cal == 500) {
